...
const Section2 = () => {
  const { Introduction, IntroPhoto } = useData();
  const [ListPersona, setListPersona] = useState([]);
  const [listAchievements, setListAchievements] = useState([]);

  useEffect(() => {
    const newListPersona = IntroPhoto?.archivement;
    setListPersona(newListPersona);

    const newlistAchievements = IntroPhoto?.titles;
            ...
            <div className="border rounded-xl">
              <div className="flex flex-col justify-between">
                <div className="flex p-5  justify-between">
                  ...
                  <span className=""></span>
                  <p className="">{Introduction.yearofbirth}</p>
                </div>

                <div className="flex p-5 justify-between gap-10 border-t">
                  <p>Ngày gia nhập ngành</p>
                  <span className=""></span>
                  <p className="">{Introduction.start}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-3 ">
            <h3 className="text-[30px]">{Introduction.name}</h3>
      ...
      <div className=" rounded-xl bg-white my-20 d:w-[1150px] d:h-[1000px] p:w-screen p:h-auto">
        <div className="d:p-6 p:p-2  flex items-center justify-center">
          <Swiper
            ...
            className="mySwiper"
          >
            {listAchievements?.map((items: any) => (
              <SwiperSlide>
                <div className=" ">
                  <img
                    src={items.image}
